...
def test_get_leaderboard():
    with get_session() as db:
        ...
        db.add(Team(slack_team_id='a', slack_team_domain='test.com'))
        db.flush()
        db.add(Channel(team_id=1, slack_channel_id='b', slack_channel_name='c', rankings_reset_at=now))
        db.flush()
        db.add(AppUser(team_id=1, slack_user_id='p_1', slack_user_name='u_1'))
        db.flush()
        db.add(AppUser(team_id=1, slack_user_id='p_2', slack_user_name='u_2'))
        db.flush()
        db.add(AppUser(team_id=1, slack_user_id='p_3', slack_user_name='u_3'))
        db.flush()
        db.add(Match(channel_id=1, winner_id=1, loser_id=2, timestamp=now))
        db.flush()
        db.add(Match(channel_id=1, winner_id=1, loser_id=3, timestamp=now))
        db.flush()
